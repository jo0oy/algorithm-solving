...
public class 당구연습 {
    ...
    public int[] solution(int m, int n, int startX, int startY, int[][] balls) {
        ...
        for(int[] ball : balls) {
            ...
            // 해당 공을 맞출 수 있는 경우의 거리 제곱근을 담을 list
            List<Integer> disList = new ArrayList<>();

            // 상 대칭
            if(!(startX == ball[0] && startY < ball[1])) {
                disList.add(calculateDistance(startX, startY, ball[0], (n - ball[1]) * 2 + ball[1]));
            }

            // 하 대칭
            if(!(startX == ball[0] && startY > ball[1])) {
                disList.add(calculateDistance(startX, startY, ball[0], (-1) * ball[1]));
            }

            // 좌 대칭
            if(!(startY == ball[1] && startX > ball[0])) {
                disList.add(calculateDistance(startX, startY, (-1) * ball[0], ball[1]));
            }

            // 우 대칭
            if(!(startY == ball[1] && startX < ball[0])) {
                disList.add(calculateDistance(startX, startY, (m - ball[0]) * 2 + ball[0], ball[1]));
            }

            // 오름차순 정렬 --> 최솟값 구하기 위해
            Collections.sort(disList);

            answer[idx++] = disList.get(0);
        }
